...
export class SocialGestureDirective implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log('socialGestures initialized!');
    this.DIRECTIVE_GESTURES.filter((dGesture) => this.gestureOpts.find(({ name }) => dGesture.name === name)).map(
      (gesture) => {
